export const addCarBtn = document.getElementById("add-car-btn");
export const brandSearchBar = document.getElementById("brand-searchbar");
export const brandSearchBtn = document.getElementById("brand-search-btn");
export const brandClearBtn = document.getElementById("brand-clear-btn");
export const modelSearchBar = document.getElementById("model-searchbar");
export const modelSearchBtn = document.getElementById("model-search-btn");
export const modelClearBtn = document.getElementById("model-clear-btn");
export const yearSearchBar = document.getElementById("year-searchbar");
export const yearSearchBtn = document.getElementById("year-search-btn");
export const yearClearBtn = document.getElementById("year-clear-btn");

export const popupCard = document.getElementById("popup-card");
export const popupCardTitle = document.getElementById("popup-card-title");
export const popupCardHeader = document.getElementById("popup-card-header");
export const popupCardHiddenId = document.getElementById("popup-card-hidden-id");
export const popupCardText = document.getElementById("popup-card-text");
export const popupCardYear = document.getElementById("popup-card-year");
export const popupCardUpdateBtn = document.getElementById("popup-card-update-btn");
export const popupCardDeleteBtn = document.getElementById("popup-card-delete-btn");
export const popupCardCloseBtn = document.getElementById("popup-card-close-btn");
